		...
		res.on("end", function(){
			try{
				try{
					...
					if(data.type=='72' || data.type=='73' || data.type=='74' || data.type=='56'){
					liRunData(data, conf);
					}else{
	...
	if(LiRunLv!=0){//盈利
	 ...
	 client.query("select * from ssc_bets where type=? and actionNo=? and isDelete=0 and lotteryNo=''", [data.type, data.number], function(err, rows, fields){
		if(err){
			...
			if(a>b && qq245649162<LiRunLv){
				qq245649162++;
				log("盈利功能已打开，当前总购买金额"+b+"预计总中奖金额"+a+"系统判断当前期为亏损状态,执行第"+qq245649162+"次循环");
				restartTask(conf, 0.1);
			}else if(b==0){
				log("盈利功能已打开，当前彩种没有投注，随机提交数据");
				submitData(data, conf);
			}else{
				log("盈利功能已打开，当前总购买金额"+b+"预计总中奖金额"+a+"系统判断当前期为盈利状态，提交数据");
				qq245649162 = 0;
				submitData(data, conf);
			}
	 }});

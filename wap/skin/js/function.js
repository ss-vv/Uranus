...
function newcode(code_arr){
	...
	if(game.type==20||game.type==65||game.type==66){ //pk10开奖动画
		...
		$("#num").addClass("lhc").empty().html(_html).css({'width': 92+code_arr.length + "%"});
		$("#num li").each(function(index){
			var num = $(this);
			var color='';
		...
		$("#num").empty().html(_html).css({'width': 95+code_arr.length + "%"});//时时彩开奖动画位置	
		$("#num li").each(function(index){
			...
			if(!data){
				...
				try{
					...
					if(type==73||type==74||type==78){
						$('.pk10_ul_line').removeClass().addClass('k8').css({"width":"100%"});
					}else{
						$('.pk10_ul_line').removeClass().addClass('klsf').css({"width":"100%"});
					}
...
function safeBeforSetPwd(){
	if(!this.oldpassword.value){xingcai("请输入旧登入密码");return false;}
	if(this.oldpassword.value.length<6){xingcai("旧登入密码至少6位");return false;}
	if(!this.newpassword.value){xingcai("请输入新登入密码");return false;}
	if(this.newpassword.value.length<6){xingcai("新登入密码至少6位");return false;}
	if(!this.qrpassword.value){xingcai("请确认新登入密码");return false;}
...
function safeBeforSetCoinPwd(){	
	if(!this.newpassword.value){xingcai("提款密码不符合规则，请重新输入");return false;}
	if(this.newpassword.value.length<6){xingcai("提款密码至少6位数");return false;}	
	var confirmpwd=$(':password.confirm', this).val();
	//if(confirmpwd!=this.newpassword.value){xingcai("两次输入密码不一样");return false;}	
	return true;
}
...
function safeBeforSetCoinPwd2(){	
	if(!this.oldpassword.value){xingcai("请输入旧提款密码");return false;}
	if(this.oldpassword.value.length<6){xingcai("旧提款密码至少6位");return false;}
	if(!this.newpassword.value){xingcai("请输入新提款密码");return false;}
	if(this.newpassword.value.length<6){xingcai("新提款密码至少6位");return false;}
	if(!this.qrpassword.value){xingcai("请确认新提款密码");return false;}
	var confirmpwd=$(':password.confirm', this).val();
	if(confirmpwd!=this.newpassword.value){xingcai("两次输入的新提款密码不一样");return false;}
	return true;
}
...
function safeBeforSetCBA(){
	if(!this.bankId.value){xingcai("请选择银行类型");return false;}
	if(!this.countname.value){xingcai("支行地址没有填写");return false;}
	if(!this.username.value){xingcai("开户姓名没有填写");return false;}
	if(!this.account.value){xingcai("银行卡号没有填写");return false;}
	if(this.account.value.length<16){xingcai("银行卡号至少16位数");return false;}
	if(!this.coinPassword.value){xingcai("请输入提款密码");return false;}
	if(this.coinPassword.value.length<6){xingcai("提款密码至少6位数");return false;}
	return true;
}
/**
 * 修改银行信息调用
...
function teamBeforeAddMember(){
	...
	if(!this.username.value){xingcai("请输入会员帐号");return false;}
	if(!/^\w{5,16}$/.test(this.username.value)){xingcai("用户名由5到16位的字母或数字组成");return false;}
	if(!this.password.value){xingcai("请输入登入密码");return false;}
...
function demo(){  $('.is_combine').attr("checked",false);}
/**
 * 投注后置函数
 */

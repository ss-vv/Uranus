			...
			<?php
				...
				foreach($times['data'] as $var){
					if($para['actionNo']){
                        ...
                        if($this->type==73||$this->type==72||$this->type==56) {
                            // 重庆彩特殊处理
                            $number = 10000 + $var['actionNo'];
                            if ($var['actionNo'] > 1380) {
                                $number = date('Ymd', strtotime(date('Y-m-d', $date - 1 * 24 * 60 * 60))) . substr($number + 10000, 1);
                            } else {
                                $number = date('Ymd', $date) . substr($number + 10000, 1);
                        ...
                        elseif($this->type==1){
							...
							// PK10
							$number = 44*(strtotime(date('Y-m-d', $date))-strtotime('2019-02-20'))/3600/24+$var['actionNo']+729788-1;
							$sql="select * from {$this->prename}data where type={$this->type} and number='$number'";
							$data=$this->getRow($sql);							
						}elseif($this->type==60){
							//广东快乐10分
							...
							//北京快乐8和PC蛋蛋
							$number = 179*(strtotime(date('Y-m-d', $date))-strtotime('2019-02-20'))/3600/24+$var['actionNo']+936980-1;
							$sql="select * from {$this->prename}data where type={$this->type} and number='$number'";
							$data=$this->getRow($sql);							
						}elseif($this->type==21){
							//广东11选五

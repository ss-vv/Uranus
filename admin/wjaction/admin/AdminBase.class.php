...
class AdminBase extends CObject{
	...
	function __construct($dsn, $user='', $password=''){
		...
		try{
			...
			$sql="update ssc_sysadmim_session set accessTime={$this->time} where id=?";
	...
	public function getPlayeds2(){
		...
		return $this->playeds2=$this->getObject($sql, 'id', null, $this->expire);
	}
	...
	public function getDateCount($date=null){
		...
		$sql="select sum(money) betAmount, sum(bonus) zjAmount from {$this->prename}bets where kjTime between $date and $date+24*3600 and lotteryNo<>'' and isDelete=0";
		$all=$this->getRow($sql);

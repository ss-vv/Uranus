$(function() {
    ...
    function t() {
        "object" != typeof $LAB ? setTimeout(t, 2e3) : initNoticeData(function(e) {
            ...
            else $("#noticeDom").html('<marquee scrollamount="4" scrolldelay="30" onmouseover="this.stop();" onmouseout="this.start();" >欢迎来到大通</marquee>')
        })
    }
    if ($("#lb_Index").length) {
        $("#lb_Index").unslider({
    ...
    $("#float").Float({topSide: 0}),
	$("#float1").Float({topSide: 0}),
    $("#float").find("#float_close a").click(function() {
        $("#float").hide()
    });
    $("#float1").find("#float_close1 a").click(function() {
        $("#float1").hide()
    });
    var u = {
        sec: 88,

...
var PullMsg = {
    ...
    getTotalAllMoney: function () {
        $.ajax({
            type: "GET",
            url: "/game/getListData.do",
            dataType: "json",
            success: function (data) {
                if (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        $.ajax({
                            type: "GET",
                            url: "/game/getLotteryData.do?gameId="+data[i].id,
                            dataType: "json",
                            success: function (data) {
                                if (data) {
                                    totalWinMoney+=data.totalTotalMoney
									PullMsg.totalWinMoney = totalWinMoney;
									if(totalWinMoney!=0){
										RequestIssue.showWinMoney();
									}
								}
...
function isThere(a) {
    if (a <= 0) {
        ...
        if (UserBet.gameId == 70 || UserBet.gameId == 74) {
            ...
            if (UserBet.gameId != 70 && a > 10 * 60 || UserBet.gameId != 74 && a > 10 * 60) {
                return true
            } else {
                return false
            }
